...
class M_database_model extends CI_Model
{
  ...
  // $field = variable name you want to read
  // ------------------------------------------------------------------------
  public function join_data_profile()
  {
    $this->db->select('*');
    $this->db->from('tb_data_diri');
    $this->db->join('tb_universitas', 'tb_universitas.id_account = tb_data_diri.id_account');
    $this->db->join('tb_pekerjaan', 'tb_pekerjaan.id_account = tb_data_diri.id_account');
    return $this->db->get();
  }
